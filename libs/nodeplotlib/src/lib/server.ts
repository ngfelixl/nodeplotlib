...
export class Server {
  ...
  private async serveData(req: IncomingMessage, res: ServerResponse) {
    if (req && req.url && /data\/[0-9]+/.exec(req.url)) {
  ...
  private serveWebsite(req: IncomingMessage, res: ServerResponse) {
    if (req && req.url && req.url.match(/plots\/[0-9]+\/index.html/)) {
      ...
      readFile(join(__dirname, '..', '..', '..', '..', '..', 'dist', 'apps', 'web', 'index.html'), 'utf-8', (error, file) => {
        if (error) {
          res.writeHead(404);
   ...
   */
  private close() {
    const pending = Object.values(this.plotsContainer).reduce(
      (a, b) => a || b.pending, false
    );
