...
import { IPlotContainer } from './models';

export class Server {
  private instance: HttpServer;
  private plotContainer: IPlotContainer = {};
  private port: number;
   ...
   * data address.
   */
  public spawn(plotContainer: IPlotContainer, cb?: () => void) {
  ...
  private openBrowserWindow() {
    for (const plotEntry of Object.entries(this.plotContainer)) {
  ...
  private serveData(req: IncomingMessage, res: ServerResponse) {
    if (req && req.url && req.url.match(/data\/[0-9]+/)) {
      ...
      const container = this.plotContainer[id];
      const temporaryPlots = container && container.plots;

      this.plotContainer[id].opened = true;
      this.plotContainer[id].pending = false;
   ...
   */
  private close() {
    const pending = Object.values(this.plotContainer)
      .reduce((a, b) => a || b.pending, false);
    const opened = Object.values(this.plotContainer)
